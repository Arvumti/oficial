...
_LTracker.push({'logglyKey': 'c60f84b6-502c-4d16-a004-7b4ef06567f1', 'sendConsoleErrors': true, 'tag': 'app-las'});
...
function readURL(input, image) {
	...
	if (input.files && input.files[0]) {
		...
		var contains = _.contains(formats_img, ext);

		if(!contains) {
			app.ut.message({text:'Solo se admiten formatos de tipo PNG o JPG', tipo:'warning'});
			dfd.reject();
			return dfd;
		}
...
function bases(){
	...
	if (typeof String.prototype.toShortDate != 'function') {
		...
		String.prototype.toShortDate = function () {
			...
			if(indexT > -1) {
				str_fecha = str_fecha.substr(0, indexT);
			}
			str_fecha += 'T06:00:00.000Z';
...
function utilerias() {
	...
	function linkFoto(json) {
		var baseUrl = '/assets/db_imgs/perfilDefault.png';

		var _currFoto = json.foto || baseUrl,
			_elem = json.elem,
			_type = json.type || 'active',
			_field = json.field,
			_url = json.url || '/application/save_foto',
			_done = json.done || fnFake,
			_error = json.error || fnFake,
			_always = json.always || fnFake
			;
		...
		function fnFake(data) {
			console.log(data);
			return {};
		}

		function click_foto(e) {
			e.preventDefault();

			var that = this;
			var elem = $(e.currentTarget);

			_file.off('change').on('change', function() {
				debugger
				var tmppath = URL.createObjectURL(this.files[0]);
				var url = _img.attr('src');
				if(url.length > 0 && url.indexOf('data:image') != 0)
					_oldUrl = url;

				var dfd = readURL(this, _img);
				if(that.isNew) {
					that.fileFake = _file;
					dfd.reject();
					return;
				...
				dfd.done(function(url) {
					debugger
					if(_type == str_options.remote.active)
						uploadImg();
				});
			});
		...
		function uploadImg() {
			...
			var padre = _file.parents('.up-foto');
			var clone = _file.clone(true);
			debugger
			var form = $('<form enctype="multipart/form-data"></form>').append(_file);
			var jData = new FormData(form[0]);
			var jParams = _params;
			jParams.field = _field;

			if(_oldUrl.indexOf('data:image') == 0)
				jParams.oldUrl = '';
			else if(_oldUrl == baseUrl)
				jParams.oldUrl = _oldUrl = '';
			else
				jParams.oldUrl = _oldUrl.split('/assets/db_imgs/')[1] || '';

			jData.append('data', JSON.stringify(jParams));

			padre.find('.img-wrap').append(clone);
			_file = clone;
			...
			function always(data) {
				...
				if(_type == str_options.remote.passive)
					setConf({url:baseUrl});
			}

			xhr.done(done).fail(fail).always(always);
		...
		var tmp_foto = app.templates.foto(json);
		_elem.html(tmp_foto).find('.upl-button').on('click', click_foto);

		var _img = _elem.find('.up-img-foto');
		var _file = _elem.find('[type="file"]');

		function setConf(json) {
			if(json.url)
				_img.attr('src', json.url);
			if(json.type)
				_type = json.type;
			if(json.idKey)
				_params.idKey = json.idKey;
		}
		function getUrl() {
	...
	function Bind(view) {
		//setTimeout(function() {
			Mousetrap.reset();
			if(view && view.binds)
				Mousetrap.bind(view.binds, view);
		//}, 1000);
	}
...
function loadAsync(options) {
	...
	var dirsJS = ['/assets/templates/' + modulo + '.js?bust='+bust];

	for(var i=0; i<dependencias.length; i++) {
		dirsJS.push('/assets/templates/' + dependencias[i] + '.js?bust='+bust);
	}

	var dfd = $.Deferred();

	if(!app.views[modulo]) {
		require(['text!/assets/templates/' + modulo + '.html'], function(data) {
			if(app.views.noAccess && data == '-1') {
				...
				require(dirsJS, function (async) {
					...
					if(app.currView.binds)
						Mousetrap.reset().bind(app.currView.binds, app.currView);
