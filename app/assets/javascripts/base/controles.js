...
var ViGrid = Backbone.View.extend({
    ...
    initialize: function(options) {
        ...
        if(this.tagName == 'div') {
            ...
            this.tmpHead = Handlebars.compile('<tr>{{#if isComAct}}<th class="gv-col-command"></i></th>{{/if}}{{#columns}}<th style="width:{{width}}px" data-field="{{field}}" {{#join}}data-join="{{table}}::{{field}}"{{/join}} class="gv-order"><strong><span>{{nombre}} </span><input type="text" class="gv-filter" placeHolder=" Buscar"/></strong></th>{{/columns}}</tr>');
